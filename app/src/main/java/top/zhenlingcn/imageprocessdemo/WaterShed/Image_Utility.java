        ...
        for(int j=0;j<height;j++){
            for(int i=0;i<width;i++){
                ...
                if(i>0&&j>0&&j<height-1&i<width-1){
                    ...
                    int grayRGB0=sourceImage.getPixel(i-1, j-1)>>16;
                    int grayRGB1=sourceImage.getPixel(i-1, j)>>16;
                    int grayRGB2=sourceImage.getPixel(i-1, j+1)>>16;
                    int grayRGB3=sourceImage.getPixel(i, j-1)>>16;
                    int grayRGB4=sourceImage.getPixel(i, j)>>16;
                    int grayRGB5=sourceImage.getPixel(i, j+1)>>16;
                    int grayRGB6=sourceImage.getPixel(i+1, j-1)>>16;
                    int grayRGB7=sourceImage.getPixel(i+1, j)>>16;
                    int grayRGB8=sourceImage.getPixel(i+1, j+1)>>16;
...
//				System.out.println(grey);
    ...
    public static void imageToFile(Bitmap image,File file){
        ...
        /*原代码使用ImageIo输出图片，此处改用FileOutputStream输出图片
        try {
            if(!file.exists()){
                file.createNewFile();
            }
